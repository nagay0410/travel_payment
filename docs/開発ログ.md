...
### APIレスポンス共通化 (フロントエンド)
- **問題**:
  - ナビゲーションバーにログインユーザー名が表示されない問題が発生。
  - 原因: フロントエンドとバックエンドのAPIレスポンス構造の不一致。
    - バックエンド: `data.userName`, `data.token` (フラット構造)
    - フロントエンド: `data.user.username`, `data.accessToken` (ネスト構造) を期待
- **対応**:
  - `api-response.model.ts` 作成: バックエンドの `ApiResponse<T>` に対応する共通型。
  - `AuthService` 修正:
    - `AuthResponse` を削除し、`ApiResponse<AuthenticationResult>` を使用。
    - `AuthenticationResult` をバックエンドに合わせて定義。
    - `login` メソッドでレスポンスから正しくユーザー情報を抽出。
- **検証**:
  - 関連する単体テストが全て成功。
