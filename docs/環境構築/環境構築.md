...
#### 環境変数の設定（ローカル開発）
ローカル開発では、以下のいずれかの方法でデータベース接続情報を設定してください。

**方法1: 環境変数を設定（推奨）**
```powershell
$env:ConnectionStrings__DefaultConnection = "Host=localhost;Database=travel_payment_db;Username=travel_payment;Password=your_password_here"
dotnet run --project src/API/API.csproj --urls http://0.0.0.0:5000
```

**方法2: appsettings.Development.json に追記**
`src/API/appsettings.Development.json` に以下を追加してください（このファイルはGit管理されています。パスワードをコミットしないよう注意）：
```json
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Database=travel_payment_db;Username=travel_payment;Password=your_password_here"
  }
}
```
...
## 9. Docker による一括起動 （Optional）
Docker を利用すると、データベース、API、Web 全てを以下のコマンドのみで起動できます。

### 環境変数ファイルの作成
初回のみ、以下の手順で環境変数ファイルを作成してください。

1. プロジェクトルートにある `.env.example` をコピーして `.env` を作成
   ```powershell
   copy .env.example .env
   ```
2. `.env` ファイルを開き、データベースパスワードを設定
   ```
